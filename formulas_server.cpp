...
//			polynom.print();
	...
	bool DNF::decide(std::vector<int> fd, int sockListener)
	{
		//разрешение ДНФ на сервере пока недоступно(для простоты написания кода)
		if(fd.empty())
		{
			throw std::runtime_error("Not allowed to calculate alone");
		}
		...
		table=get_format_table(DNF_polynoms, fd, sockListener);	
